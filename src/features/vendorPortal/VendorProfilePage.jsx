...
import { Edit3, KeyRound, Info, CheckCircle2, X, Landmark, Receipt, Building2, UserCheck, ShieldCheck } from 'lucide-react';
...
export default function VendorProfilePage() {
  ...
  const handleSaveProfile = (e) => {
    e.preventDefault();
    updateProfile({ contactPerson, phone, email });
    setIsEditing(false);
  };
  ...
  return (
    <div className="space-y-4 font-sans max-w-4xl mx-auto pb-12 antialiased">
      {/* Header & Edit Button */}
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 px-3.5 py-2 border border-teal-300 bg-teal-50 text-[#0d7676] font-bold text-xs rounded-xl hover:bg-teal-100/80 transition shadow-2xs"
        >
      ...
      <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs space-y-5">
        <div className="flex items-start gap-4 border-b border-slate-100 pb-4">
          <div className="w-12 h-12 rounded-2xl bg-teal-50 text-[#0d7676] text-xl font-extrabold flex items-center justify-center border border-teal-200 shrink-0 shadow-2xs">
            J
          </div>
        ...
        {isEditing ? (
          <form onSubmit={handleSaveProfile} className="space-y-4">
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button
                type="submit"
                className="px-4 py-2 bg-[#0d7676] hover:bg-[#0f766e] text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow-xs"
              >
                Save Profile
              </button>
            </div>
      ...
      <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8 text-xs font-medium">
          <div>
            ...
            <span className="text-slate-800 font-bold">{vendorProfile.gstin}</span>
          </div>

          <div>
            <span className="text-slate-400 block mb-0.5">PAN</span>
            <span className="text-slate-800 font-bold">{vendorProfile.pan}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8 text-xs font-medium">
          <div>
            ...
            <span className="text-slate-800 font-bold">{vendorProfile.bankName}</span>
          </div>

          <div>
            <span className="text-slate-400 block mb-0.5">Branch</span>
            <span className="text-slate-800 font-bold">{vendorProfile.branch}</span>
          </div>

          <div>
            <span className="text-slate-400 block mb-0.5">Account Number</span>
            <span className="text-slate-800 font-bold font-mono">{vendorProfile.accountNumber}</span>
          </div>

          <div>
            <span className="text-slate-400 block mb-0.5">IFSC Code</span>
            <span className="text-slate-800 font-bold font-mono">{vendorProfile.ifscCode}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs flex items-center justify-between">
        ...
        <button
          ...
          className="px-4 py-2 border border-slate-300 text-slate-700 hover:bg-slate-50 font-bold text-xs rounded-xl transition shadow-2xs"
        >
          Change
        </button>
      </div>
          ...
          <div className="bg-white rounded-2xl p-6 max-w-md w-full space-y-4 shadow-xl border border-slate-100">
            ...
            {passSuccess ? (
              ...
              <form onSubmit={handlePasswordSubmit} className="space-y-4">
                {passError && (
                  <p className="text-xs font-bold text-rose-600 bg-rose-50 p-2 rounded-lg">{passError}</p>
                )}
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-[#0d7676]"
                  />
                </div>
                ...
                <div className="flex justify-end gap-2 pt-2">
                  ...
                  <button
                    type="submit"
                    className="px-4 py-2 bg-[#0d7676] hover:bg-[#0f766e] text-white text-xs font-bold uppercase rounded-xl"
                  >
                    Update Password
                  </button>
                </div>

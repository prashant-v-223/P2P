...
function AssignmentDetail({ blId }) {
  ...
  const handleDownload = (doc) => {
    const item = typeof doc === 'string' ? { fileUrl: doc } : (doc || {});
    downloadDocumentFile(item.fileUrl || item.fileName, item.originalFilename || item.fileName || item.docType);
  };
  const saveBoe = async (event) => { event.preventDefault(); const hasExistingBoeDocument = bl.documents?.some((doc) => doc.docType === 'Customs Bill of Entry'); if (!boe.boeNumber.trim()) return setError('BOE Number is required.'); if (!boeFile && !hasExistingBoeDocument) return setError('BOE document is required.'); setSaving(true); setError(''); try { await uploadBoe(bl.blId, { ...boe, dutyAmount: Number(boe.dutyAmount) || 0, file: boeFile }); showToast({ type: 'success', title: 'BOE Saved', description: 'The Bill of Entry number and document are now linked to this BL.' }); setBoeFile(null); await load(); } catch (e) { setError(e.message); } finally { setSaving(false); } };
  const saveDocument = async () => { if (!docType || !docFile) return setError('Select a customs document type and file.'); setSaving(true); setError(''); try { await uploadCustomsDocument(bl.blId, { docType, file: docFile }); setDocType(''); setDocFile(null); showToast({ type: 'success', title: 'Document Uploaded', description: 'The file was stored securely and linked to this BL.' }); await load(); } catch (e) { setError(e.message); } finally { setSaving(false); } };
  const clear = async () => { const hasBoe = bl.documents?.some((doc) => doc.docType === 'Customs Bill of Entry'); if (!bl.boeNumber?.trim()) return setError('Save the BOE number before customs clearance.'); if (!hasBoe) return setError('Upload the BOE document before customs clearance.'); if (!window.confirm(`Confirm customs clearance for BL ${bl.blNumber}? This action is final and locks customs documents.`)) return; setSaving(true); setError(''); try { await markAsCleared(bl.blId, notes); await fetchAssignedBls(agentUser.agentId); showToast({ type: 'success', title: 'Customs Cleared', description: 'The vendor can now submit logistics invoices.' }); await load(); } catch (e) { setError(e.message); } finally { setSaving(false); } };
  if (!bl) return <div className="rounded-2xl border bg-white p-12 text-center shadow-sm">{error ? <div className="space-y-3"><p className="text-sm font-bold text-rose-700">{error}</p><button type="button" onClick={load} className="rounded-lg bg-[#0d7676] px-4 py-2 text-xs font-bold text-white">Try again</button></div> : <div role="status"><Loader2 className="mx-auto h-6 w-6 animate-spin text-[#0d7676]" /><p className="mt-3 text-xs font-bold text-slate-600">Loading BL assignment...</p></div>}</div>; const cleared = bl.status === 'custom_cleared'; const hasBoeDocument = bl.documents?.some((doc) => doc.docType === 'Customs Bill of Entry');
  return <div className="space-y-4"><Link to="/customs-agent/bl-entries" className="inline-flex items-center gap-1 text-xs font-bold text-[#0d7676]"><ArrowLeft className="h-4 w-4" />BL Assignments</Link><div><h1 className="text-xl font-extrabold">BL: {bl.blNumber}</h1><p className="text-xs text-slate-500">{bl.rfqNumber || bl.rfqId} · {bl.vendorName} · {bl.containerCount} containers</p></div>{error && <div className="rounded-xl bg-rose-50 p-3 text-xs font-bold text-rose-700">{error}</div>}<section className="grid gap-4 rounded-2xl border bg-white p-5 text-xs shadow-sm sm:grid-cols-3"><div><p className="text-slate-400">Assigned At</p><strong>{dateText(bl.assignedAt)}</strong></div><div><p className="text-slate-400">BOE Number</p><strong>{bl.boeNumber || 'Not uploaded'}</strong></div><div><p className="text-slate-400">Customs Cleared</p><strong className={cleared ? 'text-emerald-700' : 'text-amber-700'}>{cleared ? dateText(bl.customsClearedAt) : 'Pending'}</strong></div></section>{!cleared && <form onSubmit={saveBoe} className="rounded-2xl border border-teal-200 bg-white p-5 shadow-sm"><h2 className="text-sm font-extrabold">Upload Bill of Entry (BOE)</h2><p className="mt-1 text-xs text-slate-500">A saved BOE number and document are mandatory before clearance.</p><div className="mt-4 grid gap-3 sm:grid-cols-3"><input value={boe.boeNumber} onChange={(e) => setBoe({ ...boe, boeNumber: e.target.value })} placeholder="BOE number" className={inputClass} /><input type="file" accept=".pdf,.png,.jpg,.jpeg" aria-label="Select BOE document" onChange={(e) => setBoeFile(e.target.files?.[0] || null)} className={inputClass} /></div><button disabled={saving} className="mt-3 rounded-lg bg-[#0d7676] px-4 py-2.5 text-xs font-bold text-white disabled:opacity-50">Save BOE</button></form>}<section className="overflow-hidden rounded-2xl border bg-white shadow-sm"><h2 className="border-b px-5 py-4 text-sm font-extrabold">Documents ({bl.documents?.length || 0})</h2>{bl.documents?.map((doc, index) => <div key={`${doc.fileUrl}-${index}`} className="flex items-center justify-between border-b px-5 py-3 text-xs last:border-0"><span className="flex items-center gap-2"><FileText className="h-4 w-4 text-[#0d7676]" />{doc.docType}</span><div className="flex items-center gap-4"><span className="text-slate-500">{doc.fileUrl}</span><span className="text-slate-400">{dateText(doc.uploadedAt)}</span><button onClick={() => handleDownload(doc.fileUrl)} className="flex items-center gap-1 rounded-lg border px-3 py-1 text-xs font-bold hover:bg-slate-50"><Download className="h-3 w-3" />Download</button></div></div>)}</section>{!cleared && <section className="rounded-2xl border bg-white p-5 shadow-sm"><h2 className="text-sm font-extrabold">Upload Customs Documents</h2><div className="mt-3 grid gap-3 sm:grid-cols-[1fr_1fr_auto]"><SearchableSelect options={['Duty Calculation', 'Assessment Order', 'Examination Report', 'Out of Charge', 'Other Customs Document']} value={docType} onChange={(val) => setDocType(val)} placeholder="Select type" size="md" searchable={false} /><input type="file" accept=".pdf,.png,.jpg,.jpeg" aria-label="Select customs document" onChange={(e) => setDocFile(e.target.files?.[0] || null)} className={inputClass} /><button onClick={saveDocument} type="button" disabled={saving || !docType || !docFile} className="rounded-lg bg-[#0d7676] px-4 py-2 text-xs font-bold text-white disabled:opacity-50">Upload</button></div></section>}<section className={`rounded-2xl border p-5 ${cleared ? 'border-emerald-200 bg-emerald-50' : 'border-teal-200 bg-teal-50'}`}><h2 className="text-sm font-extrabold">{cleared ? 'Customs Clearance Completed' : 'Mark as Customs Cleared'}</h2>{cleared ? <p className="mt-2 text-xs text-emerald-700">Clearance completed. Vendor logistics invoicing is enabled.</p> : <><p className="mt-1 text-xs text-slate-600">Review all documents before confirming. This action locks customs uploads.</p><textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Clearance notes (optional)" className={`${inputClass} mt-3`} /><button onClick={clear} disabled={saving || !bl.boeNumber || !hasBoeDocument} className="mt-3 rounded-lg bg-[#0d7676] px-4 py-2.5 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-40"><CheckCircle2 className="mr-1 inline h-4 w-4" />{saving ? 'Processing...' : 'Confirm Customs Cleared'}</button>{(!bl.boeNumber || !hasBoeDocument) && <p className="mt-2 text-xs font-semibold text-amber-700">Save both the BOE number and BOE document before marking this BL as cleared.</p>}</>}</section></div>;
}

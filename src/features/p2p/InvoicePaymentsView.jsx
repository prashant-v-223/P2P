...
const getFormattedDueDate = (inv) => {
  if (!inv) return '—';
  const rawDue = inv.paymentDueDate || inv.dueDate || inv.expectedPaymentDate;
  if (rawDue) {
  ...
  const baseDateStr = inv.blDate || inv.invoiceDate || inv.createdAt || inv.documentDate;
                    ...
                    <tr key={inv.invoicePaymentId || inv._id || index} className="hover:bg-slate-50/70 transition-colors text-xs">
                      ...
                      <td className="py-3 px-3.5 whitespace-nowrap text-center">
                        {(() => {
                          ...
                          const activeRole = formatRoleName(inv.assignedApproverRole || inv.assignedApproverName || 'Purchase Manager');
                          return (

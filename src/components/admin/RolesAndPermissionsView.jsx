...
export default function RolesAndPermissionsView() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const fetchedPerms = permissionsData.permissions || [];
      setPermissions(fetchedPerms.length > 0 ? fetchedPerms : FALLBACK_PERMISSIONS);
      setSelectedRoleId((current) => current || rolesData.roles?.[0]?.id || null);
    } catch (error) {
      console.warn('Fallback permissions used:', error.message);
  ...
  const modules = useMemo(() => {
    const map = new Map();
    const activePerms = (permissions.length > 0 ? permissions : FALLBACK_PERMISSIONS).filter((permission) => permission.status === 'Active');
